	...
	<view class="header">
		...
		<view class="navbar" :class="{'navbar-mini':!matchLeftWindow,'popup-menu':popupMenuOpened}">
			<view class="navbar-left pointer">
				...
				<el-button type="primary" plain @click="navigateTo('/pages/index/index')">首页</el-button>
			</view>
			...
			<view class="navbar-right pointer">
				...
				<view class="navbar-menu">
					...
					<template v-if="userInfo.username">
						<!-- #ifdef H5 -->
						<view class="menu-item debug pointer" v-if="logs.length" @click="showErrorLogs">
							<svg class="svg-icon ">
						...
						<view class="menu-item">
							<!--display: none;@change="change_city" -->
							<uni-data-picker ref="picker" style="width: 200px;" v-model="city" placeholder="请选择地址"
								...
								<view class="" v-if="show">
									<text style="margin-right: 50px;">{{city}}</text>
...
<script>
	...
	import $ from "./location/jquery-3.6.0.js"
	...
	export default {
		...
		methods: {
			onchange(e) {
				...
				const value = e.detail.value[1].text.match(/(\S*)市/)[1]
				console.log(value)
				uni.setStorageSync('city', value);
				this.show = false
				...
				db.collection("companies")
					...
					.then((res) => {
						...
						for (var i = 0; i < res.result.data.length; i++) {
							this.citys[i] = res.result.data[i].city
						}
						this.citys = this.deletefunc(this.citys)
...
<style lang="scss">
	...
	/deep/ .logo uni-text[data-v-2a93fcfb] {
		font-weight: 900;
		font-size: 20px;
	}
